...
def totp_setup(
    ...
    """Generate a TOTP secret for the current user. Returns a provisioning URI
    the client can render as a QR code (e.g. Google Authenticator)."""
    import pyotp
    ...
    return TOTPSetupOut(secret=secret, otpauth_url=uri)
...
def totp_enable(
    ...
    """Verify a TOTP code generated by the user's authenticator and switch on
    enforcement. Once enabled, login requires both password and TOTP."""
    import pyotp
...
def totp_disable(
    ...
    """Disable 2FA — must provide a current valid code (proof of possession)."""
    import pyotp

    if not current_user.totp_enabled:
        return {"ok": True}
    if not pyotp.TOTP(current_user.totp_secret or "").verify(payload.code, valid_window=1):
        raise HTTPException(status_code=400, detail="Invalid TOTP code")
